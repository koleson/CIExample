...
- (void)tapRowInTableViewWithAccessibilityLabel:(NSString*)tableViewLabel atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        if (indexPath.section >= tableView.numberOfSections) {
            [self failWithError:[NSError KIFErrorWithFormat:@"Section %ld is not found in '%@' table view", (long)indexPath.section, tableViewLabel] stopTest:YES];
        }
        
        if (indexPath.row >= [tableView numberOfRowsInSection:indexPath.section]) {
            [self failWithError:[NSError KIFErrorWithFormat:@"Row %ld is not found in section %ld of '%@' table view", (long)indexPath.row, (long)indexPath.section, tableViewLabel] stopTest:YES];
        }
